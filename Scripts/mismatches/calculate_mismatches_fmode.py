...
def compute_mismatch_pair_dyntides(args):
    ...
    h_UR = generate_waveform_dyn(m1, m2, L1, L2, Fmodes=int(f+2))
...
def generate_waveform_dyn(m1, m2, L1, L2, Fmodes):
...
with Pool(n_cores) as pool:
    results = pool.map(compute_mismatch_pair_dyntides, mass_pairs_grid)
...
np.savetxt(f'{outdir}/{EOS}_MM_fmode.txt', MM)
np.savetxt(f'{outdir}/{EOS}_MM_fmode_masses.txt', np.array(mass_pairs_out))
